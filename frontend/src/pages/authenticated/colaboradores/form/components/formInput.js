...
function FormInput({ inputConfig, formik }) {
  ...
  const input = (inputConfig, formik) => {
    ...
    const existInputs = ['password', 'password', 'currency', 'mask', 'cep'].includes(inputConfig.type)
    const inputsType = {
      'password': (
        <SuiInput
          id={inputConfig.name}
      ...
      [(!existInputs) ? inputConfig.type : 'alreadyExist']: (
        <SuiInput
          id={inputConfig.name}
          type={inputConfig.type}
          name={inputConfig.name}
    ...
    return inputsType[inputConfig.type]
  }
