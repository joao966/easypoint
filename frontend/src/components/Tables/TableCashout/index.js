...
function Table({
  ...
  const renderColumns = columns.map(({ label, name, align, width }, key) => {
    ...
    return (
      ...
      >
        {(label === 'EDITAR' && success) || (name === 'checkbox' && success) ? null : label}
      </SuiBox>
    );
  });

  const handleChangeCheck = ({ target }, row, name) => {
    if (name !== 'checkbox') return;
    if (target.checked) {
      setDataCheckedState((prev) => [...prev, row]);
    }
    if (!target.checked) {
      setCheckAll(false);
      const aux = dataCheckedState.filter((cur) => {
        if (cur.documentId !== row.documentId) return true;
        if (cur.amount !== row.amount) return true;
        return false;
      });
      setDataCheckedState(aux);
    }
  };
  ...
  const renderRows = page.map((row, key) => {
    ...
    const tableRow = columns.map(({ name, align }) => {
      ...
      const template = (
        <SuiBox
          onClick={(e) => handleChangeCheck(e, row, name)}
          key={uuidv4()}
          component="td"
          p={1}
          textAlign={align}
        ...
        >
          ...
          >
            ...
            ) : row[name] === 'checkbox' && !success ? (
              <div>
                <Checkbox
                  key={row['documentId']}
                  name={row['documentId']}
                  value={key}
                  checked={dataCheckedState?.some((cur) => {
                    return cur.documentId === row.documentId && cur.amount === row.amount;
                  })}
                  inputProps={{ 'aria-label': 'controlled' }}
                />
              </div>
            ) : (
              <>{row[name] === 'edit' || row[name] === 'checkbox' ? null : row[name]}</>
            )}
          </SuiTypography>
        </SuiBox>
      );
